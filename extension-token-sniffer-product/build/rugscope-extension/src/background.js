...
async function enrichResultWithMarketData(result, options = {}) {
  ...
  const market = await fetchGeckoMarketData(result, trackedWallets);
  const enriched = {
    ...result,
    chart: market.chart,
...
async function fetchGeckoMarketData(result, trackedWallets) {
  ...
  try {
    ohlcv = await fetchJson(`https://api.geckoterminal.com/api/v2/networks/${encodeURIComponent(network)}/pools/${encodeURIComponent(poolAddress)}/ohlcv/minute?aggregate=5&limit=96&currency=usd&token=base&include_empty_intervals=true`);
  } catch (error) {
  ...
  const candles = parseOhlcvCandles(ohlcv);
...
function parseWalletTradeMatches({ trades, candles, result, trackedWallets }) {
  ...
  for (const trade of tradeRows) {
    ...
    const priceUsd = numberOrNull(kind === "sell" ? attrs.price_from_in_usd : attrs.price_to_in_usd);
